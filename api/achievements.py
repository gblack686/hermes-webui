"""Hermes Achievements engine for the WebUI (plan B11).

Native port of 9119's ``plugins/hermes-achievements`` dashboard plugin.
Follows the ``api/kanban_bridge.py`` precedent: the pure-Python engine
(ACHIEVEMENTS catalog, analyzers, tier evaluation, snapshot/checkpoint
cache) lives here and is exposed over the WebUI's plain BaseHTTPServer
dispatch (``handle_achievements_get`` / ``handle_achievements_post``)
using the shared ``j()`` / ``bad()`` helpers — not FastAPI.

Source-of-truth re-point
------------------------
The original plugin scanned Hermes Agent's ``hermes_state.SessionDB``.
This port scans the WebUI's own JSON session store via
``api.webui_session_db.WebUIJsonSessionDB`` so it depends on NO
``hermes_state`` import (the webui pytest source-dependency audit would
flag that), and adapts the message-schema drift between the two stores:

  * ``tool_calls[].name`` (or ``tool_calls[].function.name``) for
    OpenAI-style assistant tool calls, PLUS Anthropic-style ``tool_use``
    parts embedded in a list ``content``.
  * ``role == 'tool'`` result rows contribute to text/error metrics but
    are not double-counted as new tool calls.
  * ``created_at`` / ``updated_at`` / ``last_message_at`` timestamps
    (the webui store has no ``started_at`` / ``last_active`` columns).

State/snapshot/checkpoint caches live under the webui state dir
(``STATE_DIR/achievements/``) rather than ``~/.hermes/plugins/...``.
"""
...
def _state_root() -> Path:
    """Achievement caches live under the webui state dir (never inside the repo).

    Falls back to ``~/.hermes/webui/achievements`` when api.config is
    unavailable (defensive — keeps the engine importable in bare tests).
    """
    try:
        from api.config import STATE_DIR
        return Path(STATE_DIR) / "achievements"
    except Exception:
        return Path.home() / ".hermes" / "webui" / "achievements"
...
def snapshot_path() -> Path:
    return _state_root() / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return _state_root() / "scan_checkpoint.json"
...
def session_fingerprint(meta: Dict[str, Any]) -> Dict[str, Any]:
    """Cheap change-detection key for the warm-scan checkpoint cache.

    Re-pointed to the webui session-store metadata (``created_at`` /
    ``updated_at`` / ``last_message_at`` / ``message_count``) — the
    original used hermes_state's ``started_at`` / ``last_active``.
    """
    return {
        "last_active": meta.get("last_message_at") or meta.get("updated_at"),
...
def _tool_name_from_call(call: Any) -> Optional[str]:
    """Extract a tool name from an OpenAI-style tool_call dict.

    Handles both the persisted ``{"function": {"name": ...}}`` shape and
    the webui 'live' ``{"name": ...}`` shape (api/streaming.py).
    """
...
def _iter_tool_call_names(msg: Dict[str, Any]) -> List[str]:
    """Collect every tool-call name a single message represents.

    Webui session JSON carries tool invocations in two shapes:

    * OpenAI-style assistant messages: ``msg["tool_calls"]`` — each entry
      exposes the name via ``name`` or ``function.name``.
    * Anthropic-style assistant messages: a list ``content`` whose parts
      include ``{"type": "tool_use", "name": ...}``.

    ``role == 'tool'`` result rows are intentionally NOT counted here —
    they echo a call that already appeared on the assistant turn, and
    counting them would double the tool totals.
    """
    names: List[str] = []
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for msg in messages:
        ...
        for name in _iter_tool_call_names(msg):
            tool_names.add(name)
...
def scan_sessions(
    ...
    """Scan WebUI JSON sessions and build per-session achievement stats.

    ``limit=None`` (the default) scans the ENTIRE session history. A
    positive ``limit`` samples the most-recent N sessions (the
    WebUIJsonSessionDB already returns them newest-first), used by smoke
    tests.

    Warm scans stay cheap: the checkpoint cache stores per-session stats
    keyed by the session fingerprint and only re-analyzes sessions whose
    fingerprint changed. Cold scans on large histories run on a background
    thread (see ``evaluate_all``) so the dashboard UI never blocks.

    ``progress_callback(partial_sessions, scanned_so_far, total)`` fires
    every ``progress_every`` sessions so background scans can publish
    intermediate snapshots and surface badges incrementally.
    """
    try:
        from api.webui_session_db import WebUIJsonSessionDB
    except Exception as exc:
        return {"sessions": [], "aggregate": {}, "error": f"Could not import WebUIJsonSessionDB: {exc}", "scan_meta": {"mode": "failed", "sessions_total": 0, "sessions_rescanned": 0, "sessions_reused": 0}}
    ...
    try:
        sessions_meta = db.list_sessions()
    except Exception as exc:
        return {"sessions": [], "aggregate": {}, "error": f"Could not list sessions: {exc}", "scan_meta": {"mode": "failed", "sessions_total": 0, "sessions_rescanned": 0, "sessions_reused": 0}}

    if limit is not None and limit > 0:
        sessions_meta = sessions_meta[:int(limit)]
    ...
    for idx, meta in enumerate(sessions_meta, start=1):
        ...
        if isinstance(cached_stats, dict) and cached_fp == fp:
            ...
            payload = db.read_session(sid) or {}
            messages = payload.get("messages") if isinstance(payload, dict) else None
            if not isinstance(messages, list):
                messages = []
            stats = analyze_messages(sid, meta.get("title") or "Untitled", messages)
            rescanned += 1

        stats["session_id"] = sid
        stats["title"] = meta.get("title") or stats.get("title") or "Untitled"
        stats["started_at"] = meta.get("created_at")
        stats["last_active"] = meta.get("last_message_at") or meta.get("updated_at")
        stats["source"] = meta.get("session_source") or meta.get("source_tag") or meta.get("source_label")
        if meta.get("model"):
    ...
    with _SCAN_LOCK:
        ...
        def _publish_partial(partial_sessions, scanned_so_far, total):
            ...
            try:
                ...
                # _SNAPSHOT_CACHE_AT to "now". The UI treats partial results
                # as stale so it keeps polling /scan-status and sees the final
                # snapshot when the scan finishes.
...
def evaluate_all(force: bool = False) -> Dict[str, Any]:
    ...
    * ``force=True`` (manual /rescan) → run synchronously, block the
    ...
    # Lazy-load persisted snapshot from disk so fresh process starts don't
    # have to wait for a scan to serve cached data.
    ...
    # First-ever run on this machine — no snapshot yet.
    _start_background_scan()
    return _build_pending_snapshot(now)


# ---------------------------------------------------------------------------
# HTTP payload shapers + WebUI dispatch (kanban_bridge precedent)
# ---------------------------------------------------------------------------
_PREFIX = "/api/plugins/hermes-achievements/"


def _achievements_payload(force: bool = False) -> Dict[str, Any]:
    data = evaluate_all(force=force)
    payload = {
        k: data[k]
        for k in ["achievements", "unlocked_count", "discovered_count", "secret_count", "total_count", "error", "generated_at"]
        if k in data
    }
...
def _recent_unlocks_payload() -> Dict[str, Any]:
    data = evaluate_all()
    unlocks = sorted(
        [a for a in data.get("achievements", []) if a.get("unlocked")],
        key=lambda a: a.get("unlocked_at") or 0,
        reverse=True,
    )[:20]
    return {"unlocks": unlocks, "generated_at": data.get("generated_at")}


def _session_badges_payload(session_id: str) -> Dict[str, Any]:
    data = evaluate_all()
    session = next((s for s in data.get("sessions", []) if s.get("session_id") == session_id), None)
    if not session:
...
def _reset_state_payload() -> Dict[str, Any]:
    global _SNAPSHOT_CACHE, _SNAPSHOT_CACHE_AT
    save_state({"unlocks": {}})
...
def handle_achievements_get(handler, parsed) -> bool:
    """Dispatch an achievements GET.

    Returns ``True`` when a path matched (the inner handler already sent the
    response via ``j()``/``bad()``), or ``False`` when no path matched so the
    caller can emit a 404. Mirrors the kanban_bridge dispatch contract.
    """
    path = parsed.path
    try:
        if path == _PREFIX + "achievements":
            return j(handler, _achievements_payload()) or True
        if path == _PREFIX + "scan-status":
            return j(handler, _scan_status_payload()) or True
        if path == _PREFIX + "recent-unlocks":
            return j(handler, _recent_unlocks_payload()) or True
        sessions_prefix = _PREFIX + "sessions/"
        if path.startswith(sessions_prefix) and path.endswith("/badges"):
            session_id = unquote(path[len(sessions_prefix):-len("/badges")]).strip("/")
            if not session_id or "/" in session_id:
                return False
            return j(handler, _session_badges_payload(session_id)) or True
        return False
    except Exception as exc:
        return bad(handler, f"achievements error: {exc}", status=500) or True


def handle_achievements_post(handler, parsed, body=None) -> bool:
    """Dispatch an achievements POST. See ``handle_achievements_get``."""
    path = parsed.path
    try:
        if path == _PREFIX + "rescan":
            return j(handler, {"ok": True, **_achievements_payload(force=True)}) or True
        if path == _PREFIX + "reset-state":
            return j(handler, _reset_state_payload()) or True
        return False
    except Exception as exc:
        return bad(handler, f"achievements error: {exc}", status=500) or True
