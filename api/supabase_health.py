...
RELATION_SPECS: list[dict[str, str]] = [
    ...
    {"name": "host_job_runs", "kind": "base_table", "family": "runs", "filter": "lower(coalesce(client_slug::text, '')) = {tenant}", "latest": "created_at", "bad": "count(*) filter (where lower(coalesce(status::text, '')) in ('failed','error','blocked','partial'))"},
